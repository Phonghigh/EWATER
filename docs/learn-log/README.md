...
| Task | Report | Notes |
|---|---|---|
| Follow-up | [Monitoring: situation dashboard + linked interaction](FOLLOWUP-2026-07-24-monitoring-situation-linked.md) | Lift-state-up brushing so table ↔ map ↔ chart share one `selectedCode`/`hoveredCode`; a `flyNonce` counter to re-trigger `map.flyTo` when the value is unchanged; honest 60s auto-refresh off a wall-clock-anchored step; CSS-remount value flash via React `key={value}`; stale-closure-safe DOM marker handlers via refs so highlighting never rebuilds markers |
